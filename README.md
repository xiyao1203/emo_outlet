...
| 模块 | 技术栈 | 目录 |
|------|--------|------|
| **前端 App** | Flutter (Android + Windows + macOS + Web) | [`emo_outlet_app/`](emo_outlet_app) |
| **后端 API** | Python FastAPI + SQLAlchemy 2.0 (async) + SQLite/MySQL | [`emo_outlet_api/`](emo_outlet_api) |
| **AI 引擎** | 多 Provider 支持 (OpenAI / DeepSeek / 通义千问 / Mock) | `emo_outlet_api/app/services/` |
...
- **情绪报告** — 周 / 月 / 年度情绪趋势统计
- **安全防护** — 敏感词过滤 + 高风险自动中断 + 内容审计日志
- **用户偏好设置** — 历史记录、方言偏好、通知提醒等个性化配置
- **用户反馈系统** — 在线客服概览 + 反馈提交
...
| 对象 | GET/PUT | `/api/targets/{id}` | 对象详情/编辑 |
| 对象 | POST | `/api/targets/{id}/generate-avatar` | AI 生成形象 |
| 会话 | POST | `/api/sessions` | 创建会话 |
| 会话 | GET | `/api/sessions` | 历史会话列表 |
| 会话 | GET | `/api/sessions/active` | 获取活跃会话 |
| 会话 | GET | `/api/sessions/{id}` | 会话详情 |
| 会话 | POST | `/api/sessions/{id}/end` | 结束 + 情绪分析 |
| 消息 | GET | `/api/sessions/{id}/messages` | 获取消息列表 |
| 消息 | POST | `/api/sessions/{id}/messages` | 发送消息 + AI 回复 |
| 海报 | POST | `/api/posters/generate` | 生成海报 |
| 海报 | GET | `/api/posters` | 我的海报列表 |
| 报告 | GET | `/api/posters/report/overview` | 情绪报告 |
| 支持 | GET | `/api/support/overview` | 在线客服概览 |
| 支持 | POST | `/api/support/feedback` | 提交反馈 |
| 支持 | GET/PUT | `/api/support/preferences` | 用户偏好设置 |
...
| 表名 | 说明 | 核心字段 |
|------|------|---------|
| `user` | 用户表 | phone/email/游客/每日次数限制/年龄段 |
...
| `support_feedback` | 反馈表 | 用户ID/反馈内容/来源/状态 |
| `user_preference` | 偏好表 | 历史记录开关/方言/通知偏好 |
| `user_profile_detail` | 个人详情表 | 签名/性别/生日/地区 |
| `consent_record` | 用户同意记录 | 同意类型/版本/IP |
| `content_audit_log` | 内容审计日志 | 风险等级/匹配关键词/处理动作 |
